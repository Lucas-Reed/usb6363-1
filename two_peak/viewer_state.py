...
class ViewerState:
    ...
    def preview_scan_centering(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        breakpoints = [int(value) for value in raw_breakpoints] if raw_breakpoints else [count // 4, 3 * count // 4]
        if len(indices) != 2 or indices[0] == indices[1] or any(not 0 <= i < count for i in indices):
            raise ValueError("请先在波形上选择两个不同的峰 P1/P2")
        if len(breakpoints) != 2 or not 0 <= breakpoints[0] < breakpoints[1] < count:
            raise ValueError("请填写当前波形下降段的两个拐点")
        selection = dict(centering_peaks=[dict(index=i, label=f"P{n+1}") for n, i in enumerate(indices)],
                         fit=dict(breakpoints=breakpoints))
